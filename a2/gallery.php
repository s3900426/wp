<?php
$title = "Gallery";
include('includes/db_connect.inc');
include('includes/header.inc');
include('includes/nav.inc');

$sql = "SELECT skill_id, title, image_path,category FROM skills ORDER BY skill_id";
$sqlCat = "SELECT DISTINCT category FROM skills ORDER BY created_at";
$records = $conn->query($sql);
$recordsCat = $conn->query($sqlCat);
$currentCat = isset($_POST['category']) && $_POST['category'] != '' ? $_POST['category']:'none';
?>

<main class="container">
    <h1>Skills Gallery</h1>
    <form method="post" action="" class="" enctype="multipart/form-data" id="categorySelect">
        <div class="mb-3">
        <label for="category" class="form-label">Filter by category</label>
        <select class="form-select form-control w-25" id="category" name="category" onchange="this.form.submit()">
            <?php
            echo'<option value="none" '.($currentCat == $row['category'] ? 'selected' : '').'>None</option>';
            foreach ($recordsCat as $row) {
                echo '<option value="'.$row['category'].'" '.($currentCat == $row['category'] ? 'selected' : '').'>'.$row['category'].'</option>';
            }
            ?>
        </select>
    </div>
    </form>
    
    <div class="row">
        <?php
        if ($currentCat == 'none') {
            foreach ($records as $row) {
                echo '<div class="col-6 col-md-3 p-2">';
                echo '<img class="img-fluid gallery rounded" src="' . $row['image_path'] . '" alt="' . $row['title'] . '" data-bs-toggle="modal" data-bs-target="#imageModal">';
                echo ' <p><a href="details.php?skill_id=' . $row['skill_id'] . '">' . $row['title'] . '</a></p>';
                echo '</div>';
            }
        } else{
            foreach ($records as $row) {
                if ($row['category'] == $currentCat) {
                    echo '<div class="col-6 col-md-3 p-2">';
                    echo '<img class="img-fluid gallery rounded" src="' . $row['image_path'] . '" alt="' . $row['title'] . '" data-bs-toggle="modal" data-bs-target="#imageModal">';
                    echo ' <p><a href="details.php?skill_id=' . $row['skill_id'] . '">' . $row['title'] . '</a></p>';
                    echo '</div>';
                }
            }

        }

        ?>
        <div class="modal fade" id="imageModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-body">
                        <img id="modalImage" class="img-fluid" alt="Preview" src="#">
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                    </div>
                </div>
            </div>
        </div>
        <script src="assets/scripts.js"></script>
</main>

<?php
include('includes/footer.inc')

    ?>